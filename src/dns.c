...
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_dns_resolver_search, 0, 1, IS_VOID, 0)
	ZEND_ARG_OBJ_INFO(0, query, Concurrent\\DNS\\Query, 0)
ZEND_END_ARG_INFO();
...
void async_dns_ce_register()
{
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent\\DNS", "Config", async_dns_config_functions);
	async_dns_query_ce = zend_register_internal_class(&ce);
	async_dns_query_ce->ce_flags |= ZEND_ACC_FINAL;

	INIT_NS_CLASS_ENTRY(ce, "Concurrent\\DNS", "Query", async_dns_query_functions);
	async_dns_query_ce = zend_register_internal_class(&ce);
	async_dns_query_ce->ce_flags |= ZEND_ACC_FINAL;
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent\\DNS", "Resolver", async_dns_resolver_functions);
