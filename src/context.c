...
ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_context_with_isolated_output, 0, 0, Concurrent\\Context, 0)
ZEND_END_ARG_INFO();
...
void async_context_ce_register()
{
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent", "Context", async_context_functions);
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent", "ContextVar", async_context_var_functions);
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent", "CancellationHandler", async_cancellation_handler_functions);
	...
	INIT_NS_CLASS_ENTRY(ce, "Concurrent", "CancellationException", empty_funcs);
